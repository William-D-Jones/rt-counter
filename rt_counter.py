...
paMain.add_argument("-m","--pathToHits",type=str,help="If present, writes \
        ...
        if a match is found to a BED entry, with 3 added optional fields: \
...
paMain.add_argument("-x","--pathToFails",type=str,help="If present, writes \
        alignments from the BAM file (no headers) to this path and filename \
        if the alignment is not counted toward any BED entry, with 1 added \
...
paMain.add_argument("-cm","--countMultimappers",
        ...
        metavar="MODE_CHOICE")
paMain.add_argument("-co","--countOverlappers",choices=rt._counting.COUNTMODES,
        help="".join(["Mode for counting alignments that overlap multiple \
        features. Choose from [",COUNTMODES_DISP,"]. ",
        "Defaults to ",str(D_COUNTMODE),"."]),default=D_COUNTMODE,
        metavar="MODE_CHOICE")
...
dfResult=rt._counting._calc_rt(
        ...
        countOverlappers=paNames.countOverlappers)
